...
import AccHeaderKurs from './AccHeaderKurs';
import { addSubject, patchSubject } from '../../hooks/AdminApi';
...
const AccordionAdmin = ({ data, edit, setIsPost }) => {
    ...
    // initial values
    const initialValueObjs = { name: "", file: "" }

    // initial values state
    const [initialValues, setInitialValues] = useState(initialValueObjs)

    const { mutate: postMutate, isSuccess: postSuccess } = addSubject()
    const { mutate: patchMutate, isSuccess: patchSuccess } = patchSubject()
    ...
    const validationSchema = Yup.object({
        ...
        file: Yup.string().required("Ma'lumot kiritilmadi"),
    })
        ...
        <>
            <Accordion open={open === 1}>
                ...
                <AccordionBody className="border-b  flex flex-col gap-2 px-3 py-2 border-gray-400">
                    <AddSubjectForm post={post} validationSchema={validationSchema} onSubmit={onSubmit} initialValues={initialValues} handleOpen={handleOpenModule} open={openModule} />
